...
2. **Generate Design Draft**:
   ...
   - **Class Diagram rule**: The detailed UML Class Diagram is generated automatically by a separate LangGraph agent pipeline and stored in `diagram.json` alongside this design document. **Do NOT** generate UML Class Diagrams in Mermaid or text format within `design.md`. Instead, focus on high-level architecture diagrams, system boundaries, sequence diagrams, component diagrams, and data flow diagrams. Reference `diagram.json` as the authoritative source for class definitions, attributes, methods, and inter-class relationships.
